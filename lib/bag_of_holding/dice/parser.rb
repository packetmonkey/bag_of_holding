...
module BagOfHolding
  module Dice
    ...
    class Parser < Parslet::Parser
      ...
      rule(:high)       { match('[hk]') >> integer.maybe.as(:high) }
      rule(:low)        { str('l') >> integer.maybe.as(:low) }
      rule(:drop)       { str('d') >> integer.maybe.as(:drop) }
      rule(:option)     { exploding | reroll | high | low | drop }
